...
var usersSQL = sql.execute({query: "SELECT [usernames] FROM [csc155-4db].[dbo].[Users]"});

passport.use(new Strategy(function (username, password, cb) {
    process.nextTick(function () {
        for (var i = 0, len = usersSQL.length; i < len; i++) {
            var record = usersSQL[i];
            if (usersSQL.username === username) {
...
app.get("/api", passport.authenticate('local', { failureRedirect: '/login' }),
    function (req, res) {
        res.render("login", { title: "Nutrition App Login" });
        sql.execute({
            query: "SELECT [usernames] FROM [csc155-4db].[dbo].[Users]"
        }).then(function () {
